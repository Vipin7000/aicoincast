...
# --- [1. MASTER CONFIG] ---
st.set_page_config(page_title="AiCoincast v19.0 Gilded", layout="wide", initial_sidebar_state="expanded")
...
st.markdown("""
    ...
    /* 1. TOP TICKER - ABSOLUTE OVERLAY */
    .ticker-wrap { 
        width: 100% !important; background: #000; border-bottom: 2px solid #333; 
        padding: 12px 0; position: fixed; top: 0 !important; left: 0; z-index: 999999 !important;
    }
    ...
    /* RADAR GRID - FLAG STABILIZATION */
    .radar-grid { 
        display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); 
        gap: 10px; padding: 12px; border: 1px solid #00FF00; background: rgba(0,255,0,0.03); 
        border-radius: 10px; margin-top: 65px !important; margin-bottom: 15px;
    }
    .radar-box { text-align: center; font-size: 0.9rem; font-weight: bold; color: white; border-right: 1px solid #333; line-height: 1.5; }

    /* SENTINEL ALPHA - OPEN NODES (OUT OF BOX) */
    .alpha-card { background: rgba(255, 255, 255, 0.04); border-radius: 12px; padding: 15px; border-left: 5px solid #00FF00; margin-bottom: 15px; width: 100%; }
    ...
    .global-jail::-webkit-scrollbar { width: 5px; }
    .global-jail::-webkit-scrollbar-thumb { background: #00FF00; border-radius: 10px; }
...
if auth:
    ...
    with c1:
        mood_lab, mood_col = ("GREED", "#00FF00") if p_avg > 0 else (("FEAR", "#FF4B4B") if p_avg < 0 else ("NEUTRAL", "#FFFF00"))
        st.markdown(f"""<div class="header-box">
            <span class="gold-title">MARKET MOOD</span>
    ...
    with c3: st.markdown(f'<div class="header-box"><span class="gold-title">XRT Reserve Vault</span><span class="white-price">₹{total_val:,.0f}</span><span>Neural Sync Active</span></div>', unsafe_allow_html=True)

    # 3. RADAR Grid - FLAGS RE-FIXED
    ...
    # 4. SENTINEL ALPHA - OUT OF BOX GRID
    st.markdown("<h3 style='color:#00FF00; font-family:Orbitron; font-size:1.1rem; margin-bottom:15px;'>🛰️ ALPHA COMMAND COMMAND</h3>", unsafe_allow_html=True)
    cols = st.columns(3)
        ...
        with cols[idx % 3]:
            ...
            st.markdown(f"""<div class="alpha-card">
                ...
                <div style="margin-top:10px; padding-top:10px; border-top:1px solid #333; color:#FFD700; font-weight:bold; font-size:0.85rem;">
                    Holdings: {qty:,.0f} | Value: ₹{(c['current_price']*qty):,.0f}
                </div>
            </div>""", unsafe_allow_html=True)

    # 5. GLOBAL MEGA NODE - LOCKED IN BOX
    ...
    st.markdown('<div class="global-jail">', unsafe_allow_html=True)
    if data:
    ...
    st.info("🔒 Sovereign Master, initialize Gilded Terminal.")
