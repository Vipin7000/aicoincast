...
st.set_page_config(page_title="AiCoincast v18.0 Overlord", layout="wide", initial_sidebar_state="expanded")
...
st.markdown("""
    ...
    /* 1. TOP 20 TICKER - THE ABSOLUTE LOCK */
    .ticker-wrap { 
    ...
    .whale-icon { font-size: 1.2rem; margin-left: 5px; }
...
with st.sidebar:
    ...
    q_dict = {}
    for cid in CORE_IDS:
        # Default Quantity set for XRT as 369
        default_qty = 369.0 if cid == "robonomics-network" else 0.0
        q_dict[cid] = st.number_input(f"{cid[:10].upper()}", min_value=0.0, value=default_qty)
...
if auth:
    ...
    with c1:
        ...
        st.markdown(f"""<div class="header-box">
            <div style="font-size:0.6rem; opacity:0.6;">MARKET MOOD</div>
    ...
    for idx, c in enumerate(sentinel):
        with cols[idx % 3]:
            ...
            st.markdown(f"""<div class="alpha-card">
                ...
                <h2 style="margin:8px 0; font-size:1.6rem; color:#FFF;">₹{c['current_price']:,.2f}</h2>
