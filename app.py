...
# --- [SYSTEM CONFIG] Corrected Symbols & IDs ---
COIN_MAP = {
...
if m_key == "SAMASTIPUR@2026":
    ...
    with tab1:
        ...
        if data:
            ...
            for i, (id, symbol) in enumerate(COIN_MAP.items()):
                c_data = data.get(id, {})
                if c_data and c_data.get('inr') is not None:
                    p = float(c_data.get('inr', 0))
                    c = float(c_data.get('inr_24h_change', 0))
    ...
    with tab2:
        ...
        if data:
            perf_list = [{"Coin": sym, "7D %": f"{data.get(id, {}).get('inr_7d_change',0):.2f}%", 
        ...
        if st.button("Generate Bullet News"):
            if api_key:
                try:
                    ...
                    time.sleep(1) # Resource Shield
                    prompt = f"Give a short Hinglish update for {list(COIN_MAP.values())}. Focus on AI tokens."
                    st.info(model.generate_content(prompt).text)
                except: st.error("Quota full. Wait 60 seconds.")
