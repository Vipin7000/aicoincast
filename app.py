...
# --- [1. MASTER CONFIG & UI] ---
st.set_page_config(page_title="AiCoincast v172.0 Zenith", layout="wide")
...
MY_16_IDS = [
    ...
    "nftb", "virtu" # Using 'virtu' as backup for LayerAI context
]

@st.cache_data(ttl=60)
def fetch_zenith_intelligence():
    sov_res, g_res, scroll_res, idx_res = [], [], [], []
    # Explicit timeframe parameters for 24h, 7d, 30d indicators
    ...
    try:
        ...
        # 2. Sentinel Alpha (Strict 16 Nodes)
        a_raw = requests.get(f"{base_url}&ids={','.join(MY_16_IDS)}", timeout=10).json()
        if isinstance(a_raw, list): sov_res = a_raw
        
        # 3. Global Mega Node (Top 20 Display List)
        g_raw = requests.get(f"{base_url}&order=market_cap_desc&per_page=20&page=1", timeout=10).json()
        if isinstance(g_raw, list): g_res = g_raw
    except: pass
...
# --- [2. EXECUTION NODE] ---
sov_data, g_node, scroll, f2_idx = fetch_zenith_intelligence()

with st.sidebar:
    st.title("🔐 OMNI VAULT v172")
...
if m_key == MASTER_KEY:
    ...
    if scroll:
        s_html = "".join([f"<span>{safe_upper(safe_get(c, 'symbol'))}: {format_price(safe_get(c, 'current_price'))} ({get_glow_ind(safe_get(c, 'price_change_percentage_24h_in_currency'))})</span>" for c in scroll if c])
    ...
    # --- ROW 3: SENTINEL ALPHA (FIXED 16/16 + WEEK/MONTH) ---
    ...
    if sov_data:
        ...
        for c in sov_data:
            ...
            if c_id == "nftb": d_name = "PIXELREALM (NFTB)"
    ...
    st.header("🌍 Global Mega Node (Top 20 Market Index)")
    if g_node:
        dfg = []
        for c in g_node:
            dfg.append({
