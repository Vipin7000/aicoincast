...
# --- [1. MASTER CONFIG & UI] ---
st.set_page_config(page_title="AiCoincast v42.0 Final", layout="wide")
...
def fetch_elite_intelligence():
    ids_sov = ",".join(MY_12_IDS)
    try:
        # A. Sovereign 12 Dedicated (24h, 7d, 30d + Sparklines)
        ...
        # B. Global Market Context (Top 250 for Ticker & List)
...
sov_data, global_market = fetch_elite_intelligence()
...
if global_market:
    ...
    st.markdown('<div class="ticker-wrap"><marquee class="ticker-text">🔄 RE-ESTABLISHING NODE CONNECTION...</marquee></div>', unsafe_allow_html=True)

# --- [3. SIDEBAR VAULT: NIFTY & MASKED KEY] ---
with st.sidebar:
    st.title("🔐 OMNI VAULT")
...
if m_key == MASTER_KEY:
    ...
    with t1:
        # SECTION 1: MY 12 SOVEREIGN TRACKER (Snapshot 1000620217 Fix)
        ...
        if sov_data:
            ...
            for c in sov_data:
                ...
                df_sov.append({
                    ...
                    "WEEKLY (7D)": f"{get_ind(c7d)} {abs(c7d):.1f}%",
                    "MONTHLY (30D)": f"{get_ind(c30d)} {abs(c30d):.1f}%",
            ...
            st.warning("🔄 Waiting for Sovereign Node Sync...")
